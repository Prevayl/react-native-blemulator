...
import { BleErrorCode, SimulatedBleError } from "../ble-error";
...
export function errorIfUnknown(
  peripheralsById: Map<string, SimulatedPeripheral>,
...
export function errorIfConnected(
  peripheralsById: Map<string, SimulatedPeripheral>,
  peripheralIdentifier: string
): void {
...
export function errorIfNotConnected(
  peripheralsById: Map<string, SimulatedPeripheral>,
  peripheralIdentifier: string
): void {
  if (!peripheralsById.get(peripheralIdentifier)?.isConnected()) {
...
export function errorIfDisconnected(
  peripheralsById: Map<string, SimulatedPeripheral>,
  peripheralIdentifier: string
): void {
  if (!peripheralsById.get(peripheralIdentifier)?.isConnected()) {
...
export function errorIfPeripheralNotFound(
  peripheral: SimulatedPeripheral | null | undefined
): void {
  if (!peripheral) {
    const error: SimulatedBleError = new SimulatedBleError({
      errorCode: BleErrorCode.DeviceNotFound,
...
export function errorIfPeripheralNotConnected(
...
export function errorIfPeripheralDisconnected(
  peripheral: SimulatedPeripheral
): void {
  if (!peripheral.isConnected()) {
    const error: SimulatedBleError = new SimulatedBleError({
...
export function errorIfCharacteristicNotReadable(
...
export function errorIfCharacteristicNotFound(
  characteristic?: SimulatedCharacteristic
): void {
  if (!characteristic) {
    const error: SimulatedBleError = new SimulatedBleError({
...
export function errorIfMtuNegotiated(
  peripheralsById: Map<string, SimulatedPeripheral>,
  peripheralIdentifier: string
): void {
...
export function errorChecksForAccessToGatt(
...
export function errorChecksAfterOperation(
  adapterState: AdapterState,
  peripheral: SimulatedPeripheral
) {
  errorIfBluetoothNotSupported(adapterState);
...
export function errorIfNotMonitorable(
  characteristic: SimulatedCharacteristic
): void {
...
export function errorIfNotWritableWithResponse(
  characteristic: SimulatedCharacteristic
): void {
...
export function errorIfNotWritableWithoutResponse(
  characteristic: SimulatedCharacteristic
): void {
...
export function errorIfDescriptorNotReadable(
...
export function errorIfDescriptorNotFound(
  descriptor: SimulatedDescriptor | undefined | null
): void {
  if (!descriptor) {
    const error: SimulatedBleError = new SimulatedBleError({
...
export function errorIfDescriptorNotWritable(
  descriptor: SimulatedDescriptor
): void {
...
export function errorIfPayloadMalformed(value: Base64) {
  if (value.length % 4 != 0) {
    const error: Error = new Error(
      "Value is in incorrect format, Base64 string's length must be divisible by 4"
    );
    throw error;
  }
}
...
export function errorIfOperationCancelled(
  ...
  if (
    transactionMonitor.isTransactionCancelled(
      transactionId,
